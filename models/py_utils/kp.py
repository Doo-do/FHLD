...
class kp(nn.Module):
    def __init__(self,
        ...
        # self.layer5 = self._make_layer(block, res_dims[4], layers[4], stride=res_strides[4])
    ...
    def _train(self, *xs, **kwargs):
        images = xs[0]  # B 3 640 640
        masks  = xs[1]  # B 1 640 640

        p = self.conv1(images)  # B 16 320 320
        p = self.bn1(p)  # B 16 320 320
        p = self.relu(p)  # B 16 320 320
        p = self.maxpool(p)  # B 16 160 160
        p = self.layer1(p)  # B res0 160 160
        p = self.layer2(p)  # B res1 80 80
        p40 = self.layer3(p)  # B res2 40 40
        p = self.layer4(p40)  # B res3 20 20
        # p = self.layer5(p)  # B res3 20 20
        pmasks = F.interpolate(masks[:, 0, :, :][None], size=p.shape[-2:]).to(torch.bool)[0]
        pos    = self.position_embedding(p, pmasks)
        hs, encoded_feature, weights  = self.transformer(self.input_proj(p), pmasks, self.query_embed.weight, pos)
        ...
        return out, weights, [encoded_feature, p40]
...
class AELoss(nn.Module):
    def __init__(self,
        ...
        weight_dict = {'loss_ce': 3, 'loss_curves': 5, 'loss_lowers': 5, 'loss_uppers': 5, "loss_terminal_x":3}
    ...
    def forward(self,
                ...
                save,
                viz_split,
                outputs,   # pred_logits: 16x7x2, pred_curves: 16x7x8, aux_outputs
                targets): # [image, 16 * (16 x num_lane x attri): are the along axis0]

        gt_cluxy = [tgt[0] for tgt in targets[1:]] # [16 x (num_lane x attri)]
        loss_dict, indices, imgC, valid_position, gt_idx, all_boxes_num  = self.criterion(outputs, gt_cluxy)
        loss_check = {}
        weight_dict = self.criterion.weight_dict
        sums = 0
        # hard_loss_dict = ['class_error_each'+ '_each', 'loss_lowers'+ '_each', 'loss_uppers'+ '_each', 'loss_curves'+ '_each'] 
        # hard_loss = [(v * weight_dict[k[:-5]]) for k, v in loss_dict.items() if k in hard_loss_dict]
        # hard_loss = [float((hard_loss[0][i]+hard_loss[1][i]+hard_loss[2][i]).data) for i in range(hard_loss[0].shape[0])]
        for k, v in loss_dict.items():
            if k in weight_dict:
                 sums += float((v * weight_dict[k]).data)
                 loss_check[k] = float((v * weight_dict[k]).data)
        loss_check['loss'] = sums
        ...
        return (losses, loss_dict_reduced, loss_dict_reduced_unscaled,
                loss_dict_reduced_scaled, loss_value), imgC, valid_position, gt_idx,  all_boxes_num
